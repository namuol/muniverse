...
var ABANDON_MESSAGES, ACCEPT_MESSAGES, BASE_AFTERBURN, BASE_CARGO_CAP, BASE_SEED, BASE_SHIELDS, BASE_THRUST, BASE_WCHARGE_CAP, BASE_WCHARGE_RATE, BASE_WPOWER, BASE_WSPAN, BASE_WSPEED, Baddie, CabinDweller, CargoItem, CrewMission, DEFAULT_RADAR_RENDER, DIALOG_MAX_WIDTH, DOCKING_DURATION, Dialog, DialogChoice, EQUIPMENT, Equipment, FAILURE_MESSAGES, FUGITIVE_TAXI_BONUS, GAME_OVER_MSGS, GAS_GIANT_MIN_ORBIT, H, HMenu, HUD_BAR_H, HUD_BAR_W, HUD_SHIELD_X, HUD_SHIELD_Y, HUD_X, HUD_Y, Hud, ITG_INSPECT_PROB, LINE_HEIGHT, LY_SCALE, MAX_ITG_REGIONS, MAX_ITG_REGION_RADIUS, MAX_PIRATE_REGIONS, MAX_PIRATE_REGION_RADIUS, MAX_STAR_COUNT, MAX_STAR_PLANETS, MAX_STAR_RAD, MIN_ITG_REGIONS, MIN_ITG_REGION_RADIUS, MIN_ITG_STATION_PROB, MIN_PIRATE_REGIONS, MIN_PIRATE_REGION_RADIUS, MIN_PIRATE_STATION_PROB, MIN_STAR_COUNT, MIN_STAR_RAD, MIN_STATION_DIST, Menu, MenuItem, Message, Mission, NO_CABINS_MESSAGES, PARTICLES, PLANET_CLASSES, Person, Planet, Planetmap, Player, RADAR_ITEMS, RESOURCES, Radar, Resource, ResourceExchanger, STATIONS, STATION_SUB_SCREENS, SUCCESS_MESSAGES, Shot, Star, Starmap, Station, StationScreen, TURN_SPEED, TaxiMission, VMenu, W, addBaddie, addCamera, addDrone, addParticle, cam, choose, circle, clamp, closeDialog, current_planet, date, dist, flightMode, frand, gaus, getPixel, grand, groupCollides, loadColors, loadResources, main, maingame, message, planetcolors, planetmapMode, player, rand, rgba, setDialog, setPixel, split_to_lines, starcolors, starmap, starmapMode, stationMode, stationscreen, testDialog,
...
HMenu = (function(_super) {
  ...
  HMenu.prototype.render = function(xoff, y) {
    var alpha, item, left, num, width, _i, _j, _len, _len1, _ref, _ref1, _results;
    width = 0;
    _ref = this.items;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      item = _ref[_i];
...
Dialog = (function(_super) {
  ...
  Dialog.prototype.pre_blit = function(c) {
  ...
  Dialog.prototype.blit = function() {
    ...
    this.render(0, 0);
    return this.post_blit(c);
  };

  Dialog.prototype.post_blit = function(c) {};
...
StationScreen = (function(_super) {
  ...
  function StationScreen(station) {
    var d, eq, i, m, name, ncount, r, scr, tmp, _i, _j, _k, _len, _len1, _len2;
