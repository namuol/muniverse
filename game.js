...
var ABANDON_MESSAGES, ACCEPT_MESSAGES, BASE_AFTERBURN, BASE_CARGO_CAP, BASE_SEED, BASE_SHIELDS, BASE_THRUST, BASE_WCHARGE_CAP, BASE_WCHARGE_RATE, BASE_WPOWER, BASE_WSPAN, BASE_WSPEED, Baddie, BriefingDialog, CabinDweller, CargoBay, CargoItem, CrewMission, DAYS, DEFAULT_RADAR_RENDER, DIALOG_MAX_WIDTH, DOCKING_DURATION, Dialog, DialogChoice, EQUIPMENT, Equipment, FAILURE_MESSAGES, FUGITIVE_TAXI_BONUS, GAME_OVER_MSGS, GAS_GIANT_MIN_ORBIT, H, HMenu, HOURS, HUD_BAR_H, HUD_BAR_W, HUD_FTL_X, HUD_FTL_Y, HUD_SHIELD_X, HUD_SHIELD_Y, HUD_X, HUD_Y, Hud, ITG_INSPECT_PROB, LINE_HEIGHT, LY_SCALE, MAX_ITG_REGIONS, MAX_ITG_REGION_RADIUS, MAX_PIRATE_REGIONS, MAX_PIRATE_REGION_RADIUS, MAX_STAR_COUNT, MAX_STAR_PLANETS, MAX_STAR_RAD, MINUTES, MIN_ITG_REGIONS, MIN_ITG_REGION_RADIUS, MIN_ITG_STATION_PROB, MIN_PIRATE_REGIONS, MIN_PIRATE_REGION_RADIUS, MIN_PIRATE_STATION_PROB, MIN_STAR_COUNT, MIN_STAR_RAD, MIN_STATION_DIST, Menu, MenuItem, MenuStack, Message, Mission, MissionStarmap, MissionTicker, MultiMenu, NO_CABINS_MESSAGES, PARTICLES, PLANET_CLASSES, Person, Planet, Planetmap, Player, RADAR_ITEMS, RESOURCES, RGBColor, Radar, Resource, ResourceExchanger, SECONDS, STATIONS, STATION_SUB_SCREENS, SUCCESS_MESSAGES, Ship, ShipWeapon, Shot, Star, Starmap, Station, StationScreen, TURN_SPEED, TaxiMission, VMenu, W, YEARS, addBaddie, addCamera, addDrone, addParticle, cam, choose, circle, clamp, closeDialog, current_planet, current_station, date, dist, dist2, dot, drawFunds, flightMode, formatDate, formatDateShort, frand, gaus, getPixel, grand, groupCollides, lerp, loadColors, loadResources, main, maingame, menustack, message, planetcolors, planetmapMode, player, rand, rgba, setDialog, setPixel, split_to_lines, starcolors, starmap, starmapMode, startDate, stationMode, testDialog,
...
ShipWeapon = (function() {
  ...
  ShipWeapon.name = 'ShipWeapon';

  function ShipWeapon(ship, options) {
    this.ship = ship;
    this.charge = this.charge_cap = options.charge_cap;
    this.charge_rate = options.charge_rate;
  ...
  ShipWeapon.prototype.update = function() {
    if (this.charge < this.charge_cap) {
      return this.charge += this.charge_rate;
    }
  };

  ShipWeapon.prototype.fire = function() {
    var shot;
    if (!(this.charge >= this.power)) {
      return;
    }
    this.charge -= this.power;
    sounds.shot0.play();
    shot = new Shot(this.ship.x + this.ship.w / 2, this.ship.y + this.ship.h / 2, this.ship.x + this.ship.w / 2 + (this.ship.ax / this.ship.thrust) * 20000, this.ship.y + this.ship.h / 2 + (this.ship.ay / this.ship.thrust) * 20000, this.power, this.speed, 4, 'shots', this.span, this.ship.vx, this.ship.vy);
    shot.ship = this.ship;
    gbox.addObject(shot);
...
Ship = (function() {
  ...
  Ship.prototype.setAng = function(val) {
    this.ang = val;
    this.ax = Math.cos(this.ang) * this.thrust;
    this.ay = Math.sin(this.ang) * this.thrust;
    if (this.ang < 0) {
      return this.ang = Math.PI * 2 - this.ang;
    }
  };

  Ship.prototype.first = function() {
    var _this = this;
    if (this.thrusting) {
      this.vx += this.ax;
      this.vy += this.ay;
    ...
    if (this.turning !== 0) {
      this.setAng(this.ang + this.turning);
    }
    this.x += this.vx;
    this.y += this.vy;
    if (this.shot) {
    ...
    this.weapon.update();
    groupCollides(this, 'shots', function(shot) {
      var i;
      if (shot.ship === _this) {
        return;
      }
  ...
  Ship.prototype.can_flee = function() {
    var MIN_DIST, MIN_DIST2, dst2, group, groupname, k, obj, _i, _len, _ref;
    MIN_DIST = 1200;
    MIN_DIST2 = MIN_DIST * MIN_DIST;
    _ref = this.flee_groups;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      groupname = _ref[_i];
      group = gbox.getGroup(groupname);
      for (k in group) {
        if (!__hasProp.call(group, k)) continue;
        obj = group[k];
        dst2 = dist2(this, obj);
        if (dst2 < MIN_DIST2) {
          return false;
...
CargoBay = (function() {
  ...
  function CargoBay(capacity) {
    this.capacity = capacity;
  }

  CargoBay.prototype.canHold = function(items) {
    return true;
  };
...
Baddie = (function(_super) {
  ...
  function Baddie() {
    var attr, eq, weapon_options;
    Baddie.__super__.constructor.call(this, new Person);
    this.alive = true;
    this.missions = [];
    this.funds = 500;
    this.cabins = [];
    ...
    this.weapon = new ShipWeapon(this, weapon_options);
    ...
    this.hostile = Math.random() < (0.25 + starmap.current_star.pirate - starmap.current_star.itg);
    this.init();
  }

  Baddie.prototype.group = 'baddies';

  Baddie.prototype.x = 0;

  Baddie.prototype.y = 0;

  Baddie.prototype.vx = 0;

  Baddie.prototype.vy = 0;

  Baddie.prototype.init = function() {
    this.skip = false;
    this.frame = 0;
    this.tileset = 'ship0_tiles';
    this.w = 8;
    this.h = 8;
    this.x = frand(-gbox.getScreenW() * 3, -gbox.getScreenW() * 3);
    this.y = frand(-gbox.getScreenH() * 3, -gbox.getScreenH() * 3);
    this.vx = 0;
    this.vy = 0;
    this.ang = 0;
    this.ax = 0;
    ...
    return this.flee_groups = ['player', 'baddies', 'planet', 'stations'];
  };
  ...
  Baddie.prototype.first = function() {
    ...
    */
    ...
    return Baddie.__super__.first.call(this);
  };
...
Baddie = (function(_super) {
  ...
  Baddie.prototype.first = function() {
    var ax, ay, d, dx, dy, len, shot, xoff, yoff,
      _this = this;
    if (!player.alive) {
      this.hostile = false;
    }
    if (this.hostile) {
      ...
      if (d < W) {
        if (this.wcharge >= this.wcost && d < this.attack_dist) {
          ...
          shot = new Shot(this.x + this.w / 2, this.y + this.h / 2, this.tsx, this.tsy, this.wpower, this.wspeed, 1, 'shots', this.wspan, this.vx, this.vy);
          shot.ship = this;
          gbox.addObject(shot);
        }
      }
    ...
    return groupCollides(this, 'shots', function(shot) {
      var i;
      if (shot.ship === _this) {
        return;
      }
...
addBaddie = function() {
  return gbox.addObject(new Baddie);
};
...
CabinDweller = (function(_super) {
  ...
  CabinDweller.prototype.failure = function() {
    var idx;
    CabinDweller.__super__.failure.call(this);
    idx = player.cabins.indexOf(this.person);
    return player.cabins.splice(idx, 1);
  };
...
TaxiMission = (function(_super) {
  ...
  TaxiMission.prototype.text = function() {
    return TaxiMission.__super__.text.call(this) + 'Taxi lvl' + (this.lvl + 1) + ' by ' + formatDateShort(this.deadline) + ' $' + this.price;
  };
...
planetmapMode = function() {
  ...
  stopGroups = ['background', 'planet', 'player', 'baddies', 'drones', 'shots', 'resources', 'particles', 'starmap', 'stations', 'radar', 'hud'];
...
Planet = (function() {
  ...
  function Planet(star, pid, num, moon, itg, pirate) {
    ...
    if (moon) {
      this.ptype = 'moon';
      this.c1 = this.random.choose(planetcolors[2]);
      this.c2 = [this.random.choose(this.c1), this.random.choose(this.c1), this.random.choose(this.c1)];
      this.color = rgba(this.c1);
      this.atmosphere_size = 3;
      this.atmosphere_density = 0.25;
    } else if (this.orbit > GAS_GIANT_MIN_ORBIT && this.random() < PLANET_CLASSES.gas_giant.prob) {
      this.ptype = 'gas_giant';
      this.c1 = this.random.choose(planetcolors[1]);
      this.c2 = [this.random.choose(this.c1), this.random.choose(this.c1), this.random.choose(this.c1)];
      this.color = rgba(this.c1);
      this.atmosphere_size = 25;
      this.atmosphere_density = 0.5;
    } else {
      this.ptype = 'rocky';
      this.c1 = this.random.choose(planetcolors[0]);
      this.c2 = [this.random.choose(this.c1), this.random.choose(this.c1), this.random.choose(this.c1)];
      this.color = rgba(this.c1);
      this.atmosphere_size = 3;
      this.atmosphere_density = 0.3;
    }
    ...
    this.diry = 0;
    this.pre_render(this.dirx, this.diry);
    this.moons = [];
    if (moon) {
      return;
    }
  ...
  Planet.prototype._render_pass = function(ctx, radius, x, y, dirx, diry, s, w, a) {
    var bn, gn, grd, rn;
    ctx.beginPath();
    grd = ctx.createRadialGradient(x + dirx, y + diry, 0, x + dirx, y + diry, radius * 1.3);
    grd.addColorStop(0, "rgba(" + this.c1[0] + "," + this.c1[1] + "," + this.c1[2] + "," + a + ")");
    grd.addColorStop(s, "rgba(" + this.c1[0] + "," + this.c1[1] + "," + this.c1[2] + "," + ((1 - s) * a) + ")");
    rn = lerp(this.c1[0], this.c2[0], s + w);
    gn = lerp(this.c1[1], this.c2[1], s + w);
    bn = lerp(this.c1[2], this.c2[2], s + w);
  ...
  Planet.prototype.pre_render = function(dirx, diry) {
    var ctx, i, x, y, _results;
    this.el = document.createElement('canvas');
    this.el.setAttribute('width', (this.radius + this.atmosphere_size) * 2 + 1);
    this.el.setAttribute('height', (this.radius + this.atmosphere_size) * 2 + 1);
    ...
    ctx.beginPath();
    ctx.fillStyle = this.star.bg_color;
    if (this.ptype === 'gas_giant') {
      ctx.arc(x, y, this.radius + this.atmosphere_size, 0, 2 * Math.PI, false);
    } else {
      ctx.arc(x, y, this.radius, 0, 2 * Math.PI, false);
    }
    ctx.fill();
    ctx.closePath();
    this._render_pass(ctx, this.radius, x, y, dirx, diry, 0.75, 0.2, 1);
    i = 0;
    _results = [];
    while (i < (this.atmosphere_size + 1)) {
      this._render_pass(ctx, this.radius + i * 1, x, y, dirx, diry, 0.75, 0.2, this.atmosphere_density);
      _results.push(++i);
    }
    return _results;
  };

  Planet.prototype.render_outline = function(scale, x, y) {
    var ctx;
    ...
    ctx.fillStyle = this.star.bg_color;
    if (this.ptype === 'gas_giant') {
      ctx.arc(x, y, (this.radius + this.atmosphere_size) * scale + 1, 0, 2 * Math.PI, false);
    } else {
      ctx.arc(x, y, this.radius * scale + 1.5, 0, 2 * Math.PI, false);
    }
    ctx.fill();
    return ctx.closePath();
  };

  Planet.prototype.render = function(scale, x, y) {
    var ctx, h, w;
    ctx = gbox.getBufferContext();
    if (!ctx) {
      return;
    ...
    w = this.w * scale;
    h = this.h * scale;
    return ctx.drawImage(this.el, x - (w / 2), y - (h / 2), w, h);
  };
...
starmapMode = function() {
  var g, groups, stopGroups, _i, _j, _len, _len1;
  stopGroups = ['background', 'planet', 'player', 'baddies', 'drones', 'shots', 'resources', 'particles', 'planetmap', 'stations', 'radar', 'hud'];
  for (_i = 0, _len = stopGroups.length; _i < _len; _i++) {
    g = stopGroups[_i];
    gbox.stopGroup(g);
  }
...
Starmap = (function() {
  ...
  Starmap.prototype.first = function() {
    var dst, dx, dy, previous, vx, vy, x, y;
    if (this.skip) {
    ...
    if (this.closest_star && gbox.keyIsHit('a')) {
      dst = this.current_star.distance_to(this.closest_star);
      if (player.fuel() < dst) {
        message.set('Insufficient fuel.', 90);
      } else if (!player.can_flee(true)) {
    ...
    if (gbox.keyIsHit('c')) {
      sounds.cancel.play();
      flightMode(current_planet === void 0);
    }
...
flightMode = function(reset) {
  ...
  groups = ['background', 'planet', 'player', 'baddies', 'drones', 'shots', 'resources', 'particles', 'stations', 'radar', 'hud'];
  for (_j = 0, _len1 = groups.length; _j < _len1; _j++) {
    g = groups[_j];
    gbox.stopGroup(g);
    gbox.toggleGroup(g);
...
Player = (function(_super) {
  ...
  function Player(name) {
    var attr, eq, weapon_options;
    Player.__super__.constructor.call(this, new Person);
    this.alive = true;
    this.missions = [];
    this.funds = 500;
    this.cabins = [];
    ...
    this.flee_groups = ['baddies', 'planet', 'stations'];
    this.init();
  }
  ...
  Player.prototype.first = function() {
    ...
    return Player.__super__.first.call(this);
  };
  ...
  Player.prototype.can_flee = function() {
    var MIN_DIST, MIN_DIST2, dst2, group, groupname, groups, k, obj, _i, _len;
    MIN_DIST = 1200;
    MIN_DIST2 = MIN_DIST * MIN_DIST;
    groups = ['baddies', 'planet', 'stations'];
...
dot = function(color, x, y, alpha) {
  var c;
  x = clamp(x, 4, cam.w - 4) + 0.5;
  y = clamp(y, 4, cam.h - 4) + 0.5;
  c = gbox.getBufferContext();
  if (c) {
    return circle(c, color, Math.round(x), Math.round(y), 1);
  }
};

DEFAULT_RADAR_RENDER = function(x, y, alpha) {
  return dot('yellow', x, y, alpha);
};
...
RADAR_ITEMS = {
  ...
  'baddies': {
    ...
    render: function(x, y, alpha, obj) {
      if (obj.hostile) {
        return dot('red', x, y, alpha);
      } else {
        return dot('green', x, y, alpha);
      }
    }
  }
};
      ...
      _results.push((function() {
        ...
        for (id in _ref) {
          ...
          if (item.min_dist() < 0 || d < item.min_dist()) {
            ...
            if (item.render) {
              _results1.push(item.render(x, y, 1, obj));
            } else {
...
Hud = (function() {
  ...
  Hud.prototype.blit = function() {
    var c, extrax, extray, ftl_alpha, h_diff;
    c = gbox.getBufferContext();
    if (!c) {
      return;
    }
    ...
    if (player.shields > 0) {
      this.h += h_diff;
      c.fillRect(Math.round(HUD_X + HUD_SHIELD_X + 2 + extrax), Math.round(HUD_Y + HUD_SHIELD_Y + 9 + extrax), HUD_BAR_W, Math.round(this.h));
    }
    if (this.tick % 10 === 0) {
      this.can_flee = player.can_flee();
    }
    if (this.can_flee) {
      ftl_alpha = 1.0;
    } else {
...
main = function() {
  gbox.setGroups(['background', 'game', 'starmap', 'planetmap', 'menustack', 'planet', 'stations', 'resources', 'particles', 'player', 'baddies', 'drones', 'shots', 'hud', 'radar', 'message', 'dialog']);
  loadColors();
