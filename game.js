...
VMenu = (function(_super) {
  ...
  VMenu.prototype.render = function(x, yoff) {
    var alpha, height, item, num, top, _i, _j, _len, _len1, _ref, _ref1, _results;
    height = 0;
    _ref = this.items;
...
Starmap = (function() {
  ...
  Starmap.prototype.render = function(c) {
    var m, r, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _results;
    if (c) {
      c.drawImage(gbox.getImage('starmap_gui'), 0, 0);
      c.drawImage(this.starmap, 0, 0);
...
StationScreen = (function(_super) {
  ...
  function StationScreen(station) {
    ...
    for (_i = 0, _len = STATION_SUB_SCREENS.length; _i < _len; _i++) {
      ...
      this.items.push(new MenuItem(scr.name));
      this.sub_menus.push(new VMenu);
      switch (scr.name) {
        case 'Cargo':
          for (name in RESOURCES) {
            if (!__hasProp.call(RESOURCES, name)) continue;
            r = RESOURCES[name];
            this.sub_menus[i].items.push(new ResourceExchanger(name, this.station));
          }
          break;
        case 'Missions':
          this.sub_menus[i].items = this.station.missions;
          break;
        case 'Hangar':
          for (_j = 0, _len1 = EQUIPMENT.length; _j < _len1; _j++) {
            eq = EQUIPMENT[_j];
            this.sub_menus[i].items.push(new Equipment(eq));
          }
      }
      ++i;
    }
  ...
  StationScreen.prototype.first = function() {
    ...
    this.update();
    this.current_sub_menu = this.sub_menus[this.selected];
    return this.current_sub_menu.update();
  };
  ...
  StationScreen.prototype.blit = function() {
    ...
    this.current_sub_menu.render(0, 0);
    return this.render(0, 4);
  };
