...
(function() {
  var ABANDON_MESSAGES, ACCEPT_MESSAGES, BASE_AFTERBURN, BASE_CARGO_CAP, BASE_SEED, BASE_SHIELDS, BASE_THRUST, BASE_WCHARGE_CAP, BASE_WCHARGE_RATE, BASE_WPOWER, BASE_WSPAN, BASE_WSPEED, Baddie, CabinDweller, CargoItem, CrewMission, DEFAULT_RADAR_RENDER, DIALOG_MAX_WIDTH, DOCKING_DURATION, Dialog, EQUIPMENT, Equipment, FAILURE_MESSAGES, FUGITIVE_TAXI_BONUS, GAME_OVER_MSGS, GAS_GIANT_MIN_ORBIT, H, ITG_INSPECT_PROB, LY_SCALE, MAX_ITG_REGIONS, MAX_ITG_REGION_RADIUS, MAX_PIRATE_REGIONS, MAX_PIRATE_REGION_RADIUS, MAX_STAR_COUNT, MAX_STAR_PLANETS, MAX_STAR_RAD, MIN_ITG_REGIONS, MIN_ITG_REGION_RADIUS, MIN_ITG_STATION_PROB, MIN_PIRATE_REGIONS, MIN_PIRATE_REGION_RADIUS, MIN_PIRATE_STATION_PROB, MIN_STAR_COUNT, MIN_STAR_RAD, MIN_STATION_DIST, Menu, MenuItem, Message, Mission, NO_CABINS_MESSAGES, PARTICLES, PLANET_CLASSES, Person, Planet, Planetmap, Player, RADAR_ITEMS, RESOURCES, Radar, Resource, ResourceExchanger, STATIONS, STATION_SUB_SCREENS, SUCCESS_MESSAGES, Shot, Star, Starmap, Station, StationScreen, TURN_SPEED, TaxiMission, W, addBaddie, addCamera, addDrone, addParticle, cam, choose, circle, clamp, current_planet, date, dialog, dist, flightMode, frand, gaus, getPixel, grand, groupCollides, loadColors, loadResources, main, maingame, message, planetcolors, planetmapMode, player, rand, rgba, setPixel, starcolors, starmap, starmapMode, stationMode,
  ...
  Dialog = (function() {
    ...
    Dialog.prototype.set = function(str, lifespan, person) {
      if (lifespan == null) {
        lifespan = 240;
      }
      this.msgs = [];
    ...
    Dialog.prototype.blit = function(x, y) {
      if (!this.visible) {
        return;
      }
      if (this.msgs[0].person) {
        this.msgs[0].person.render_face(1, 1, 1, true);
      }
  ...
  Message = (function() {
    ...
    Message.prototype.blit = function(x, y) {
      if (!this.visible) {
        return;
      }
      if (this.msgs[0].person) {
  ...
  Menu = (function() {
    ...
    Menu.prototype.c = function() {
      if (this.items[this.selected]) {
        return this.items[this.selected].c();
      }
    };
  ...
  TaxiMission = (function(_super) {
    ...
    function TaxiMission(person) {
      ...
      this.price = RESOURCES.fuel.mean_price * 1.25;
      ...
      this.price *= starmap.current_star.distance_to(this.star);
  ...
  starmapMode = function() {
    ...
    stopGroups = ['background', 'planet', 'player', 'baddies', 'drones', 'friend_shots', 'foe_shots', 'resources', 'particles', 'planetmap', 'stations', 'radar'];
  ...
  Starmap = (function() {
    ...
    function Starmap(x, y, density) {
      var arr, c, color, count, el, i, planet, radius, star, starcount;
      this.skip = false;
      this.sector = {
        x: x,
        y: y
    ...
    Starmap.prototype.blit = function() {
      var c, m, r, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _results;
      ...
      if (c) {
        ...
        if (this.current_star) {
          ...
          circle(c, '#33e5ff', Math.round(this.current_star.x), Math.round(this.current_star.y), player.fuel() / LY_SCALE);
        }
        _ref2 = player.missions;
        _results = [];
        for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
          m = _ref2[_k];
          if (m.location) {
            _results.push(gbox.blitTile(c, {
              tileset: 'cursors',
              tile: 3,
  ...
  planetmapMode = function() {
    var g, groups, stopGroups, _i, _j, _len, _len1;
    stopGroups = ['background', 'planet', 'player', 'baddies', 'drones', 'friend_shots', 'foe_shots', 'resources', 'particles', 'starmap', 'stations', 'radar'];
    for (_i = 0, _len = stopGroups.length; _i < _len; _i++) {
      g = stopGroups[_i];
      gbox.stopGroup(g);
    }
    ...
    function Planet(star, pid, num, moon, itg, pirate) {
      ...
      for (name in RESOURCES) {
        ...
        if (res.pirate_mod_min) {
          this.prices[name] *= frand(res.pirate_mod_min, res.pirate_mod_max);
        }
        ...
        while (c < count) {
          ...
          r = this.radius * frand(res.min_dist, res.max_dist);
          x = r * Math.cos(ang);
          y = r * Math.sin(ang);
      ...
      if ((itg === this.num) || Math.random() < itg_station_prob) {
      ...
      if ((pirate === this.num) || Math.random() < pirate_station_prob) {
  ...
  flightMode = function(reset) {
    ...
    groups = ['background', 'planet', 'player', 'baddies', 'drones', 'friend_shots', 'foe_shots', 'resources', 'particles', 'stations', 'radar'];
    for (_j = 0, _len1 = groups.length; _j < _len1; _j++) {
      g = groups[_j];
      gbox.stopGroup(g);
      gbox.toggleGroup(g);
  ...
  Player = (function() {
    ...
    function Player(name) {
      ...
      this.equipment = {};
      for (_i = 0, _len = EQUIPMENT.length; _i < _len; _i++) {
        eq = EQUIPMENT[_i];
        if (!eq.no_default) {
  ...
  ResourceExchanger = (function(_super) {
    ...
    ResourceExchanger.prototype.a = function() {
      ...
      return player.cargo[this.name].push(this.station.cargo[this.name].pop());
    };
    ...
    ResourceExchanger.prototype.b = function() {
      ...
      return this.station.cargo[this.name].push(player.cargo[this.name].pop());
    };

    ResourceExchanger.prototype.text = function() {
      var lamt, ramt;
      lamt = ramt = 0;
      if (player.cargo[this.name]) {
      ...
      return "" + this.name + "[" + lamt + "] <-$" + this.price + "-> [" + ramt + "]";
    };
  ...
  stationMode = function(station) {
    var g, groups, stationscreen, stopGroups, _i, _j, _len, _len1;
    stopGroups = ['background', 'planet', 'player', 'baddies', 'drones', 'friend_shots', 'foe_shots', 'resources', 'particles', 'starmap', 'stations', 'radar'];
    for (_i = 0, _len = stopGroups.length; _i < _len; _i++) {
      g = stopGroups[_i];
      gbox.stopGroup(g);
  ...
  StationScreen = (function(_super) {
    ...
    function StationScreen(station) {
      var eq, i, m, name, r, scr, tmp, _i, _j, _k, _len, _len1, _len2;
        ...
        switch (scr.name) {
          case 'Cargo':
            for (name in RESOURCES) {
              if (!__hasProp.call(RESOURCES, name)) continue;
              r = RESOURCES[name];
              this.sub_items[i].push(new ResourceExchanger(name, this.station));
            }
            break;
          case 'Missions':
            ...
            break;
          case 'Hangar':
            for (_j = 0, _len1 = EQUIPMENT.length; _j < _len1; _j++) {
              eq = EQUIPMENT[_j];
              this.sub_items[i].push(new Equipment(eq));
            }
        }
        ++i;
      }
      ...
      for (_k = 0, _len2 = tmp.length; _k < _len2; _k++) {
        ...
        switch (m.type) {
          case 'taxi':
            if (this.station.planet.num === m.location.pnum && this.station.planet.star === m.location.star) {
              m.success();
            }
        }
      }
    ...
    StationScreen.prototype.first = function() {
      ...
      if (this.sub_screen < 0) {
        this.sub_screen = STATION_SUB_SCREENS.length - 1;
      }
    ...
    StationScreen.prototype.blit = function() {
      var alpha, c, extra, i, left, n, s, w, _i, _len;
      c = gbox.getBufferContext();
      if (!c) {
        return;
      }
      ...
      extra = STATION_SUB_SCREENS[this.sub_screen].extra_blit;
      if (extra) {
        extra(c);
      }
      left = 2;
      i = 0;
      for (_i = 0, _len = STATION_SUB_SCREENS.length; _i < _len; _i++) {
        s = STATION_SUB_SCREENS[_i];
        n = s.name;
        alpha = 0.5;
        if (this.sub_screen === i) {
          alpha = 1;
        }
        w = (n.length + 1) * 8;
